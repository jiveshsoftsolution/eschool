...
class Online_exam extends CI_Controller 
{
    ...
    public function onlineexam_student_home()
    {
        ...
        $this->load->view('onlineexam/onlineexam_student_home', $data_question);
	...
	public function attempt_question($next="")
    {
        ...
        if($this->input->post())
		{
			...
			$student_teacher_class_id = $this->examModel->get_student_teacher_class_id(58,1) ;
			$data_answer['student_teacher_class_id'] = $student_teacher_class_id[0]->student_teacher_class_id;
			$data_question['online_question'] =  $this->onlineexamModel->get_online_question_by_class_section(2,$question_no + 1);
			$existing_answer = "";
            ...
            else
			{
					if($this->input->post('answer')!= "false")
					{
						...
						$data_answer['ans_status'] = addslashes($this->input->post('ans_status'));
						insert($data_answer, "ems_online_exam_student_ans");
					}
			}
			if(isset($data_question['online_question'][0]->que_marks_id))
			$data_question['existing_answer'] =  $this->onlineexamModel->get_selected_answer($data_question['online_question'][0]->que_marks_id ) ;
			else
		...
		{		
			...
			{
			    $prev = $next;
				$data_question['online_question']=  $this->onlineexamModel->get_online_question_by_class_section(2,$prev);
