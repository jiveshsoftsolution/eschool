	  ...
	  <div class="box_c">
		...
		<div class="box_c_content">
		  <form action="<?php echo base_url()?>index.php/student/student_attendance/approve_attendance" id="frm_student_attendance_view" class="nice" method="post" onsubmit="return validate_add_exam_marks();">
		   <h3>Approve Student Attendace</h3>             
			<div class="formRow">
				<div class="row">
            ...
            <div class="twelve columns">
                <div class="box_c">
					...
					<?php if(isset($studentListForSendSms['status']) || count($studentListForSendSms)==0) 
						...
						{ ?>
					<form action="<?php echo base_url()?>index.php/student/student_attendance/add_attendance_approve_information" method="post">
							...
							<?php $count = 1;  for($i=0; $i < count($studentListForSendSms) ; $i++) {?>
								<tr>
									...
									<td>
										...
										?>
										<input type="radio" readonly="true" name="attendance[<?php echo $studentListForSendSms[$i]->student_teacher_class_id; ?>]"  checked="checked" value="P">
										<label class="label-present" for="present_<?php echo $studentListForSendSms[$i]->student_id?>">Present</label>
										<?php }
                                                                                else { ?>
											
											<input type="radio" readonly="true" name="attendance[<?php echo $studentListForSendSms[$i]->student_teacher_class_id; ?>]" disabled value="P">
											<label class="label-present" for="present_<?php echo $studentListForSendSms[$i]->student_id?>">Present</label>
										<?php }
                                                                                            }
									...
									<td>
						               <?php               if(isset($attendance_status[0])){
										if($attendance_status[0] == "A"){
											
											?>
										  <input type="radio" name="attendance[<?php echo $studentListForSendSms[$i]->student_teacher_class_id; ?>]"  checked="checked" value="A" >
										  ...
										  <?php
											...
											?>
										   <input type="radio"  name="attendance[<?php echo $studentListForSendSms[$i]->student_teacher_class_id; ?>]"  value="A" disabled>
											<label class="label-absent" for="absent_<?php echo $studentListForSendSms[$i]->student_id?>">Absent</label>
										  <?php
										}}
									...
									<td>
										...
										if($attendance_status[0] == "L"){										
											?>
											<input type="radio" readonly="true" name="attendance[<?php echo $studentListForSendSms[$i]->student_teacher_class_id; ?>]"  checked="checked" value="L" >
										  ...
										  <?php
											...
											?>
										   <input type="radio"  name="attendance[<?php echo $studentListForSendSms[$i]->student_teacher_class_id; ?>]"  value="L" disabled>
											<label class="label-leave" for="leave_<?php echo $studentListForSendSms[$i]->student_id?>">Leave</label>
										  <?php
										}
                                                                               }else{											
											?>
										   <input type="radio" readonly="true" name="attendance[<?php echo $studentListForSendSms[$i]->student_teacher_class_id; ?>]"  value="L" disabled>
										  <label class="label-leave" for="leave_<?php echo $studentListForSendSms[$i]->student_id?>">Leave</label>
										  <?php
										}
