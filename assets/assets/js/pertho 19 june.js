...
var base_path = 'http://makeroutetech.com/schoolportal/assets/assets/';
...
// ----- Global ----- //
	...
	prth_reize_el = {
		...
		init: function(){
           ...
           if(typeof yearlyAdmission != 'undefined'){

				yearlyAdmission.replot({ resetAxes: true });
			...
			if(typeof feesChart != 'undefined'){

				feesChart.replot({ resetAxes: true });

			};
			
			if(typeof teacherPerformanceSubject != 'undefined'){

				teacherPerformanceSubject.replot({ resetAxes: true });

			};
			if(typeof todayStAttendance != 'undefined'){

				todayStAttendance.replot({ resetAxes: true });

			};
			
			if(typeof todayTeacherAttendance != 'undefined'){

				todayTeacherAttendance.replot({ resetAxes: true });
	...
	prth_box_actions = {
		...
		init: function() {
			...
			$('.box_actions').each(function(){
				...
				$(this).append('<span class="bAct_hide"><img src="'+base_path+'img/blank.gif" class="bAct_x" alt="" /></span>');

				$(this).append('<span class="bAct_toggle"><img src="'+base_path+'img/blank.gif" class="bAct_minus" alt="" /></span>');
				...
				$(this).find('.bAct_toggle').on('click', function(){
					...
					if( $(this).closest('.box_c_heading').next('.box_c_content').is(':visible') ) {
						...
						$(this).html('<img src="'+base_path+'img/blank.gif" class="bAct_plus" alt="" />');

					} else {

						$(this).closest('.box_c_heading').next('.box_c_content').slideDown('slow');

						$(this).html('<img src="'+base_path+'img/blank.gif" class="bAct_minus" alt="" />');
	...
	prth_charts = {
		...
		new_admission: function() {
			...
			yearlyAdmission = $.jqplot('new_admission', [new_admission_data], {
				...
				seriesColors: ["#dc0a00","#ff7214","#ffab25","#fdd40d","#b4e409","#05da15","#0f98de","#0d5add" ,"#dc0a00"],

				title:'Admission Progress Yearly Of School',
               // highlighter: { show: true, sizeAdjust: 100.5 , tooltipLocation : 'n' },
				seriesDefaults:{
		...
		teacherPerformance_subject : function() {

			var teacherPerformance_subject_data = [

				['First Div', 40],['Second Div', 20], ['Third Div', 30],

				['Fail', 10]

			];

			teacherPerformanceSubject = $.jqplot ('teacherPerformance_subject', [teacherPerformance_subject_data],

				{
				  ...
				  seriesColors: ["#05da15","#ff7214","#0d5add","#fdd40d","#f8ff1a","#b4e409","#05da15","#0f98de","#0d5add"],
				  title:'Student Marks',
		...
		fees_chart : function() {
			...
			var fees_chart_data = [
				...
				['Class I', 12],['Class II', 9], ['Class III', 14],

				['Class IV', 16],['Class IV', 7], ['Class IV', 9]

			];

			feesChart = $.jqplot ('fees_chart', [fees_chart_data],

				{
				  ...
				  seriesColors: ["#dc0a00","#ff7214","#ffab25","#fdd40d","#f8ff1a","#b4e409","#05da15","#0f98de","#0d5add"],
				  title:'Remaining fees detail class wise of each month',
		...
		pl_plot1: function() {
			...
			var plPlot1_data = [['J.Doe', 42],['M.Lou', 16],['J.Smith', 22],['J.Adams', 35],['M.Johnson', 46] , ['J.rahul', 47]];
		...
		pl_plot6: function() {
			...
			plplot6 = $.jqplot('pl_plot6', [plPlot1_data], {
...
// ----- Misc ----- //
		...
		init: function() {
			...
			var calendar = $('#calendar').fullCalendar({
				...
				buttonText: {
					...
					prev: '<img src="'+base_path+'img/blank.gif" alt="" class="table_prev" />',

					next: '<img src="'+base_path+'img/blank.gif" alt="" class="table_next" />'
	...
	prth_nested_accordion = {
		...
		init: function() {
			...
			$('.accordion h4 a,.accordion h5 a').append('<img src="'+base_path+'img/blank.gif" />');
